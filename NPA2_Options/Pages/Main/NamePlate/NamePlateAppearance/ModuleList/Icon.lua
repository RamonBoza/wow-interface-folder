-- Author      : Nogarder
-- Create Date : 2013/3/7 15:55:15
NPA_Options.RegisterPageFunc("NamePlateAppearanceIcon",function()
	...
	listeditor.RefreshFunc=function(list)
		...
		local listdata={}
		for name,_ in pairs(NPA_Setting.NamePlate[EntryName].IconSetting)do
	...
	listeditor.OnCopyFunc=function(oldname,newname)
		NPA_Setting.NamePlate[EntryName].IconSetting[newname]=CopyTable(NPA_Setting.NamePlate[EntryName].IconSetting[oldname])
		FireEvent()
	end
	listeditor.OnDeleteFunc=function(oldname)
		NPA_Setting.NamePlate[EntryName].IconSetting[oldname]=nil
		FireEvent()
	end
	...
	listeditor.OnCreateFunc=function(newname)
		local default=NPA.SettingManager.GetDefault("NamePlateIcon")
		NPA_Setting.NamePlate[EntryName].IconSetting[newname]=CopyTable(default)
		FireEvent()
	end
	listeditor.OnClickFunc=function(name,button,self)
		local aname=name.."Icon"
		local framegroup=Preview.FrameData.ModuleMain.FrameGroup
		...
		--set edit page
		NPA_Options.SetPage("NamePlateIconEditor",main,NPA_Setting.NamePlate[EntryName].IconSetting[name],
			--on value update
			FireEvent,
			name)
	end
	...
	--
	NPA_Options.RegisterPage("NamePlateAppearanceIcon",main)
