-- Author      : Nogarder
-- Create Date : 2013/4/2 11:41:14
NPA_Options.RegisterPageFunc("NamePlateAppearanceIconGroup",function()
	...
	listeditor.RefreshFunc=function(list)
		...
		local listdata={}
		for name,_ in pairs(NPA_Setting.NamePlate[EntryName].IconGroupSetting)do
	...
	listeditor.ButtonOnEnterFunc=function(self)
		local name=self.Value.."IconGroup"
		local framegroup=Preview.FrameData.ModuleMain.FrameGroup
	...
	listeditor.OnCopyFunc=function(oldname,newname)
		NPA_Setting.NamePlate[EntryName].IconGroupSetting[newname]=CopyTable(NPA_Setting.NamePlate[EntryName].IconGroupSetting[oldname])
		FireEvent()
	end
	listeditor.OnDeleteFunc=function(oldname)
		NPA_Setting.NamePlate[EntryName].IconGroupSetting[oldname]=nil
		FireEvent()
	end
	...
	listeditor.OnCreateFunc=function(newname)
		local default=NPA.SettingManager.GetDefault("NamePlateIconGroup")
		NPA_Setting.NamePlate[EntryName].IconGroupSetting[newname]=CopyTable(default)
		FireEvent()
	end
	listeditor.OnClickFunc=function(name,button,self)
		local aname=name.."IconGroup"
		local framegroup=Preview.FrameData.ModuleMain.FrameGroup
		...
		--set edit page
		NPA_Options.SetPage("NamePlateIconGroupEditor",main,NPA_Setting.NamePlate[EntryName].IconGroupSetting[name],
			--on value update
			FireEvent,
			name)
	end
	...
	--
	NPA_Options.RegisterPage("NamePlateAppearanceIconGroup",main)
