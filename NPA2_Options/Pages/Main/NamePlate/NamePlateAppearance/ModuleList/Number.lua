-- Author      : Nogarder
-- Create Date : 2013/4/1 7:21:18
NPA_Options.RegisterPageFunc("NamePlateAppearanceNumber",function()
	...
	listeditor.RefreshFunc=function(list)
		...
		local listdata={}
		for name,_ in pairs(NPA_Setting.NamePlate[EntryName].NumberSetting)do
	...
	listeditor.OnCopyFunc=function(oldname,newname)
		NPA_Setting.NamePlate[EntryName].NumberSetting[newname]=CopyTable(NPA_Setting.NamePlate[EntryName].NumberSetting[oldname])
		FireEvent()
	end
	listeditor.OnDeleteFunc=function(oldname)
		NPA_Setting.NamePlate[EntryName].NumberSetting[oldname]=nil
		FireEvent()
	end
	...
	listeditor.OnCreateFunc=function(newname)
		local default=NPA.SettingManager.GetDefault("NamePlateNumber")
		NPA_Setting.NamePlate[EntryName].NumberSetting[newname]=CopyTable(default)
		FireEvent()
	end
	listeditor.OnClickFunc=function(name,button,self)
		local aname=name.."Number"
		local framegroup=Preview.FrameData.ModuleMain.FrameGroup
		...
		--set edit page
		NPA_Options.SetPage("NamePlateNumberEditor",main,NPA_Setting.NamePlate[EntryName].NumberSetting[name],
			--on value update
			FireEvent,
			name)
	end
	...
	--
	NPA_Options.RegisterPage("NamePlateAppearanceNumber",main)
