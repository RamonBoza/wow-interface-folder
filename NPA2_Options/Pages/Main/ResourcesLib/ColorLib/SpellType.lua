-- Author      : Nogarder
-- Create Date : 2013/4/9 14:21:50
NPA_Options.RegisterPageFunc("ColorLibSpellType",function()

	local CopyTable=CopyTable
	--localization
	local Localization=NPA_Options.Localization.PagesColorLibSpellType
	---
	local parent=NPA_Options.MainFrame
	local entry=NPA_Setting.Color["SpellType"]
	--
	local main=NPA_Options.Template.CreateFrameWithBar(parent,Localization["SpellType"],210,350,nil,nil)
	main:Hide()
	...
	--buttons
	local buttons={}
	local data=NPA_Options.GetBasicTypeSpellType()
	local i=0
	local step=-45
	...
	local Reset=function()
		local defaultcolor=NPA.SettingManager.GetDefault("Color")["SpellType"]
		for index,button in pairs(buttons)do
	...
	----
	NPA_Options.RegisterPage("ColorLibSpellType",main)
