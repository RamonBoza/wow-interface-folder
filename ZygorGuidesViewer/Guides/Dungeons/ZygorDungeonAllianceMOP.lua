...
step
kill 1 Baron Ashbury##46962 |only if default
...
step
kill 1 Lord Godfrey##46964 |only if default
...
step
label "start"
Press _I_ and queue for Blackfathom Deeps or enter the dungeon with your group |goto Blackfathom Deeps/1 44.8,10.6 |c
step
...
path	14.6,43.2
_Swim under_ the water and _follow_ the path |goto Blackfathom Deeps 14.6,43.2 < 10 |noway |c
step
kill 1 Domina##74476
...
_Swim under_ the water and follow the path |goto 55.8,63.2 < 5 |c |noway
step
...
step
kill 1 Randolph Moloch##46383 |goto 50.8,19.2 |only if default
...
path	50.0,50.4
_Follow_ the path |goto 50.0,50.4 < 5 |c |noway
step
talk Nurse Lillian##46410
...
step
use S.A.F.E. "Parachute"##60680
Walk to this ledge and use your parachute to reach the bottom safely |goto Gnomeregan/2 23.9,73.3 < 5
talk B.E Barechus##44560
...
step
talk Hooded Crusader##64738
accept Rank and File##31490 |goto Scarlet Halls/1 31.9,84.4 |only if not heroic_dung()
accept Just for Safekeeping, Of Course##31493 |goto Scarlet Halls/1 31.9,84.4 |only if not heroic_dung()
accept Rank and File##31495 |goto Scarlet Halls/1 31.9,84.4 |only if heroic_dung()
accept Just for Safekeeping, Of Course##31497 |goto Scarlet Halls/1 31.9,84.4 |only if heroic_dung()
only if not completedq(31490) and not completedq(31493) and not completedq(31495) and not completedq(31497)
step
map Scarlet Halls/1
path follow loose;loop off;ants curved
...
Defeat Houndmaster Braun |scenariogoal 19266 |goto 19266
step
map Scarlet Halls/2
path follow loose;loop off;ants curved
...
step
kill 1 Armsmaster Harlan##58632
...
step
kill 1 Flameweaver Koegler##59150
...
collect Codex of the Crusade##87267 |q Just for Safekeeping, Of Course##31493/1 |only if havequest(31493)
collect Codex of the Crusade##87267 |q Just for Safekeeping, Of Course##31497/1 |only if havequest(31497)
...
Kill 50 Scarlet Crusaders. |q Rank and File##31490/1 |only if havequest(31490)
Kill 50 Scarlet Crusaders. |q Rank and File##31495/1 |only if havequest(31495)
only if havequest(31490) or havequest(31495)
step
talk Hooded Crusader##64738
turnin Rank and File##31490 |goto 39.3,15.6 |only if not heroic_dung()
only if not completedq(31490)
step
talk Hooded Crusader##64738
turnin Just for Safekeeping, Of Course##31493 |goto 39.3,15.6 |only if not heroic_dung()
only if not completedq(31493)
step
talk Hooded Crusader##64738
turnin Rank and File##31495 |goto 39.3,15.6 |only if heroic_dung()
only if not completedq(31495)
step
talk Hooded Crusader##64738
turnin Just for Safekeeping, Of Course##31497 |goto 39.3,15.6 |only if heroic_dung()
only if not completedq(31497)
step
...
author support@zygorguides.com
step
Press _I_ and queue for Scarlet Monastery or enter the dungeon with your group |goto Scarlet Monastery/1 74.7,45.8 < 100 |c |or
Enter the dungeon |confirm |or
step
...
only if not completedq(31514) or not completedq(31516)
step
...
only if not completedq(26901) and not completedq(26907)
step
...
_Follow_ the path |goto Scholomance/1 85.4,24.1 < 5 |c
step
...
step
kill 1 Boneweaver##59193 |goto Scholomance/2 48.6,26.7
kill 1 Rattlegore##59153 |goto Scholomance/2 48.6,26.7
...
kill King Gordok##11501 |q King of the Gordok##27125/1 |goto 31.8,26.4 |only if havequest(27125)
...
click Gordok Tribute##3365
confirm |next "Return2" |only if not completedq(27124) and not completedq(27124) and not completedq(27114)
