...
function IM:BagFullCheck(event)
	...
	if barFraction < ZGV.db.profile.fullness_search then
		BagsFullPopup:Hide()
		ZGV.IM.isPopupShowing = false
	end
...
do -- limit scope of variables used for threading
	...
	local IterateStepsThread, GetDistanceToNearestVendorThread
	local sleepCount=0
	...
	function IM:GetDistanceToNearestVendor(point)
		...
		GetDistanceToNearestVendorThread = coroutine.create(GetDistanceToNearestVendorFunction)
		coroutine.resume(GetDistanceToNearestVendorThread, point)
	...
	function IM:OnThreadedEvent(event)
		...
		if IterateStepsThread then
			success, errorText = coroutine.resume(IterateStepsThread)
			if not success then 
				--error(errorText) 
			end
		...
		if GetDistanceToNearestVendorThread then
			success, errorText = coroutine.resume(GetDistanceToNearestVendorThread)
			if not success then -- Actually, I don't really care if if errors out . . .
				--print(debugstack(GetDistanceToNearestVendorThread))
				--error(errorText)
			end
		end
		...
		if sleepCount >= 30 or (event and event == "ZYGORGV_STEP_CHANGED") then
			...
			elseif lastPoint.distance > checkDistance and IM.isNearVendor == true then -- player moves out of vendor's range
				...
				if event and event == "ZYGORGV_STEP_CHANGED" then
					IM:FindRecommendedSpaceInBags(info)
				end
			end
	...
	local function Finish(returns)
		IterateStepsThread = nil
		GetDistanceToNearestVendorThread = nil
	...
	function IM:FindRecommendedSpaceInBags(returns)
		...
		IterateStepsThread = coroutine.create(iterateSteps)
		
		-- Start the thread
		success, errorText = coroutine.resume(IterateStepsThread, returns)
