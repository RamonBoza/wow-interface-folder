...
ZGV.WaypointFunctions['internal'] = {
	...
	setwaypoint = function (self,goalnumORx,y,title,only_type,extradata)  local t1=debugprofilestop()  repeat
		...
		if self.Pointer.corpsearrow or self.Pointer:IsCorpseArrowNeeded() then
			ZGV:Debug("&waypoints &_PUSH SetWaypoint: it's a corpse, bailing out.")
			break --------------------------------------------------------------------------------
		end

		-- false: DESTROY THE WAYPOINTS!
		if goalnumORx==false then
			ZGV:Debug("&waypoints &_PUSH SetWaypoint starts: clearing.")
		...
		local goal,step
		if goalnumORx then
			ZGV:Debug("&waypoints &_PUSH SetWaypoint starts: %s %s title=%s only_type=%s",goalnumORx or -1,tostring(y or -1),title or "nil",only_type or "nil")
			...
			ZGV:Debug("&waypoints &_PUSH SetWaypoint starts: showing all step waypoints.")
		end
		...
		if type(goalnumORx)=="number" and type(y)=="number" then
			ZGV:Debug("&waypoints SetWaypoint to coords: %.2f %.2f",goalnumORx,y)
			-- coords, plain as day
		...
		if waypath then
			...
			ZGV:Debug("&waypoints Waypath present in step, showing it as '%s':",pathtype)
		...
		if step and step.goals and not only_type then
			ZGV:Debug("&waypoints &_PUSH showing goal(s)...")
			if goal then
			...
			ZGV:Debug("&waypoints &_POP done showing goal(s).")
		end
		...
		ZGV:Debug("&waypoints arrowpoint %s, farmpoint %s, waypath %s",ZGV.Pointer.waypoint_tostring(arrowpoint),ZGV.Pointer.waypoint_tostring(farmpoint),waypath and #waypath>0 and "#"..#waypath or "empty")


		local function onChangeZone()
			if not UnitIsDeadOrGhost("player") and waypath and waypath.coords then
		...
		if ZGV.db.profile.pathfinding then
			...
			elseif waypath and not pointed and ZGV.Pointer.nummanual==0 then
				...
				if waypath.loop then
					...
					if currentmapid ~= waypath.coords[1].map then
						self:Debug("&waypoints We're not in the farm path's zone, let's travel.")
						ZGV.Pointer:FindTravelPath("farm")
					else
						self:Debug("&waypoints We're in the farm path's zone! Clearing travel, just follow the route.")
						LibRover:Abort()
				...
				if currentmapid~=firstcoord.map then
					self:Debug("&waypoints We're not in the path's zone, let's travel to the first point.")
					local x,y=0,0
					...
					self:Debug("&waypoints We're in the path's zone! Clearing travel, just follow the route.")
					LibRover:Abort()
					ZGV.Pointer:ClearSet("route")
					ZGV.Pointer:PointToNextInPath("path")  -- it's not looped, handle it somehow.
				end
			elseif goalnumORx~="sticky" and not step:IsDynamic() then
				LibRover:Abort()
			end
		end
	...
	until true
	self:Debug("&waypoints &_POP SetWaypoint ends [%.2f ms]",debugprofilestop()-t1)
	end,
