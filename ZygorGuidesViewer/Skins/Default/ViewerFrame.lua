...
local function Step_OnClick(self,button)
	if ZGV.CurrentStep==self.step or self.is_sticky then
	...
	if profile.showbriefsteps then
		...
		for i=1,50 do
			...
			if stepf and stepf:IsShown() then
				...
				local onaction=nil
				for j=1,30 do if ZGV.actionbuttons[i][j] and ZGV.actionbuttons[i][j]:IsMouseOver() then onaction=true break end end
				if not onaction then for j=1,30 do if ZGV.petactionbuttons[i][j] and ZGV.petactionbuttons[i][j]:IsMouseOver() then onaction=true break end end end
