...
function ZGV:CacheRep(name,standing,min,max,val,isFriend,friendThresh,isBodyguard)
	...
	rep.isFriend = not not isFriend
	rep.friendship = friendThresh
	rep.isBodyguard = not not isBodyguard
...
function ZGV:CacheReputations()
	for i=1,200 do
		...
		if name and bmin and (not header or headerhasrep) then
			...
			local rep = self:CacheRep(name,standing,bmin,bmax,val,isFriend and true,friendThresh, isBodyguard)
			rep.id = id
...
function RepProto:Current()
	return (self.isBodyguard and bodyguardrepnames[standing]) or (self.isFriend and tillerrepnames[standing]) or standingnames[standing]
end

function RepProto:Next()
	return (self.isBodyguard and bodyguardrepnames[self:GetNextStanding()]) or (self.isFriend and tillerrepnames[self:GetNextStanding()]) or standingnames[self:GetNextStanding()]
end
...
function RepProto:GetNextStanding()
	if self.isBodyguard then  return min(self.friendship+10000,2*10000)  end
...
function RepProto:GetFormattedStanding(standing)
	...
	return ("|cff%s%s|r"):format(standingcolors[standing],(self.isBodyguard and bodyguardrepnames[standing]) or (self.isFriend and tillerrepnames[standing]) or standingnames[standing])
end
