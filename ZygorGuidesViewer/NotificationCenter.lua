...
function Notification.ApplyLayout(collapsing,flashOneButton)
	...
	if not Notification.buttonLayout then Notification.buttonLayout = {}
	else wipe(Notification.buttonLayout) end
...
function Notification:OnEvent(event,arg1,...)
...
function Notification:CreateNotificationFrame()
	frame = CHAIN(CreateFrame("Frame", "Zygor_Notification_Center", UIParent))
		...
		:SetScript("OnEvent",function(self,event,...) Notification:OnEvent(event,...) end)
		:SetScript("OnEnter", function(...) private.PopOut(...) end)
		:SetScript("OnLeave", function(...) private.PopBack(...) end)
		:SetScript("OnMouseDown", function(...) private.BeginMove(...) end)
		:SetScript("OnMouseUp", function(...) private.EndMove(...) end)
		:SetScript("OnUpdate", function(...) private.OnUpdate(...) end)
	.__END
	...
	frame.blackBar = CHAIN(frame:CreateTexture("blackBar", "BACKGROUND"))
		...
		:SetTexture(0.01, 0.01, 0.01, 1.0)
		:SetDrawLayer("BACKGROUND", 1)
		:Show()
	.__END
	
	frame.Tab = frame:CreateTexture() --This is the grey bar at the bottom.
	frame.Tab:SetTexture(unpack(SkinData("MainBackdropBorderColor")))
	frame.buttons = {}
