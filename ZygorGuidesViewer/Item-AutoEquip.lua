...
function AutoEquip:GetItemScore(itemlink,invslot,verbose)
	local total = 0

	local score, code, desc = ItemScore:UsableHeirloomItem(itemlink,self.verbose)
	if score < 0 then return score,code,desc
	else total = total + score end
	
	score, code, desc = ItemScore:GetItemScore(itemlink,invslot,nil,self.verbose)
	if score < 0 then return score,code,desc
	else total = total + score end
...
function AutoEquip:ScanBagsForUpgrades(verbose,noreport,noblock)
	...
	for bag=0, NUM_BAG_SLOTS do for bagslot=1, GetContainerNumSlots(bag) do repeat
		...
		if ScoreCache[itemlink] and not self.verbose then
			...
			score, code, info = self:GetItemScore(itemlink,nil)
...
function AutoEquip:ShowPopup(item)
	...
	local function GetStatComparison(item)
		...
		if ItemScore.maxHeirloomLevels[itemid] and ItemScore.playerlevel > ItemScore.maxHeirloomLevels[itemid] then
			item.link = "|Hitem:"..itemid.."|h[q]|h"
		end
		if cur and ItemScore.maxHeirloomLevels[cur.itemid] and ItemScore.playerlevel > ItemScore.maxHeirloomLevels[cur.itemid] then
