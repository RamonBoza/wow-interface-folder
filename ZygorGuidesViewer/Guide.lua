...
function Guide:New(title,data,extra)
...
function Guide:GetCompletionText()
	...
	elseif self.completionmode=="level" then
		...
		-- ~~ Jeremiah
		return math.floor(comp*100).."%", ("Level %2d (+%d bars) reached: %d%%"):format(self.endlevel,floor((self.endlevel-math.floor(self.endlevel))*20+0.05),math.floor(comp*100))
...
function Guide:Parse(fully)
	if self.rawdata and (not self.parsed or not self.fully_parsed) then
		...
		if fully then ZGV:Debug("&startup_parse Parsing guide: "..self.title) end
					...
					if step.sticky_labels then
						...
						for i,stickylabel in ipairs(step.sticky_labels) do repeat
							if stickylabel~=step.label then
								local stickynums = self.steplabels[stickylabel]  if not stickynums then ZGV:Error("Sticky step by that label not found: %s",stickylabel) break end
								local stickynum=stickynums[1]
								local stickystep = self.steps[stickynum]  if not stickystep then ZGV:Error("Sticky step by that label/number not found: %s %s",stickylabel,stickynum)  break  end
